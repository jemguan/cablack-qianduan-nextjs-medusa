  ...
  return (
    <div
      ...
      style={{
        ...style,
        opacity,
        transition: `opacity ${transition?.duration ?? 0.6}s ease-out`,
      }}
    >
      {children}
    </div>
...
const ImageModuleComponent = memo(function ImageModuleComponent({
  ...
  const content = (
    <div className="cursor-pointer transition-all duration-300 active:scale-95 will-change-transform h-full w-full">
      <img
        src={imageUrl}
...
const CollectionModuleComponent = memo(function CollectionModuleComponent({
  ...
  return (
    <LocalizedClientLink
      href={collectionLink}
      className="block cursor-pointer transition-all duration-300 active:scale-95 w-full h-full will-change-transform"
    >
      <div className={`relative w-full h-full rounded-lg ${getGlassClassName(true)}`} style={getGlassStyle(true)}>
        ...
        {title && (
          <h3 className="text-base font-semibold text-foreground text-center drop-shadow-sm">
            {title}
          </h3>
        )}
      </div>
...
const VideoModuleComponent = memo(function VideoModuleComponent({
  ...
  return (
    <div ref={containerRef} className="relative overflow-hidden rounded-lg shadow-lg w-full h-full group">
      ...
      {/* 控制按钮 - 移动端始终显示 */}
      <div className="absolute top-2 right-2 flex gap-2 opacity-100 md:opacity-0 md:group-hover:opacity-100 transition-opacity duration-200 z-10">
        <button
          onClick={toggleMute}
...
function TextModuleComponent({
  ...
  const backgroundClass = backgroundColor 
    ? `${backgroundColor} rounded-lg p-4 ${textAlignClass} transition-opacity duration-300`
    : `${getGlassClassName(true)} rounded-lg p-4 ${textAlignClass} transition-opacity duration-300`;
  ...
  const textContent = (
    ...
    >
      ...
      {subtitle && (
        <h3 className={`${subtitleSize} font-semibold mb-2 ${subtitleColor}`}>
          {subtitle}
        </h3>
      )}
      {content && (
