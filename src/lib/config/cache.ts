...
// Next.js fetch 缓存配置
// revalidate 必须在 next 对象内部才能正确工作
export const CACHE_STRATEGIES = {
  ...
  CATEGORY: isDevelopment ? DEV_CACHE_CONFIG : { next: { revalidate: 7200 } },
  COLLECTION: isDevelopment ? DEV_CACHE_CONFIG : { next: { revalidate: 7200 } },
  REGION: isDevelopment ? DEV_CACHE_CONFIG : { next: { revalidate: 3600 } },
  BLOG: isDevelopment ? DEV_CACHE_CONFIG : { next: { revalidate: 300 } }, // Blog 缓存 5 分钟，确保内容及时更新
  BRAND: isDevelopment ? DEV_CACHE_CONFIG : { next: { revalidate: 3600 } }, // 品牌缓存 1 小时
